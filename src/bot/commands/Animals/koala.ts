...
export const Koala: Command = {
  name: "command.koala.name",
  description: "command.koala.description",
  category: "Animals",
  dms: true,
  exec: async (message, _, lang) => {
    const msg = await message.channel.send(
      Language.getNode(lang, "command.koala.finding")
    );
    let response: AxiosResponse<any>;
    try {
      response = await axios.get("https://some-random-api.ml/img/koala");
    } catch (error) {
